...
import type { ArticulacionCatalogosResponse } from '../../types/articulaciones';
...
export default function ReportesArticulacion() {
    ...
    const [areas, setAreas] = useState<ArticulacionCatalogosResponse['areas']>([]);
    ...
    useEffect(() => {
        articulacionesService.getCatalogos().then(c => {
            setAreas(c.areas || []);
                        ...
                        <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3 flex-wrap">
                                    <button
                                        onClick={async () => { setIsGeneratingExcel(true); try { const r = await articulacionesService.downloadExcel(startDate || undefined, endDate || undefined, selectedArea || undefined, selectedEstado || undefined, selectedSubdireccion || undefined); triggerDownload(r.blob, r.filename); } finally { setIsGeneratingExcel(false); } }}
                                    ...
                                    <button
                                        onClick={async () => { setIsGeneratingPdf(true); try { const r = await articulacionesService.downloadPdf(startDate || undefined, endDate || undefined, selectedArea || undefined, selectedEstado || undefined, selectedSubdireccion || undefined); triggerDownload(r.blob, r.filename); } finally { setIsGeneratingPdf(false); } }}
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                                    ...
                                    <div className="bg-white p-5 md:p-7 rounded-2xl border border-zinc-200/80 shadow-sm lg:col-span-2">
                                        <div className="flex items-center gap-2 mb-6">
                                            ...
                                            <h3 className="text-lg font-bold text-zinc-800">Articulaciones por Subdirección</h3>
                                        </div>
                                        <div className="h-[350px]">
                                            {!data?.porSubdireccion?.length ? (
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <BarChart data={data.porSubdireccion} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                        ...
                                                        <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: '#A1A1AA' }} tickLine={false} axisLine={false} />
                                                        <RechartsTooltip cursor={{ fill: '#f4f4f5', radius: 6 }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgb(0 0 0/0.1)', padding: '12px 16px' }} />
                                                        <Bar dataKey="count" fill="#06b6d4" radius={[6, 6, 0, 0]} name="Articulaciones" maxBarSize={60} isAnimationActive={false}>
