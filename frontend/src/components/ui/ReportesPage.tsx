...
interface ReportesPageService {
    getCatalogos(): Promise<{ areas: AreaItem[]; subdirecciones?: { id: string; name: string }[] }>;
    getEstadisticas(startDate?: string, endDate?: string, areaId?: string, estado?: string, subdireccionId?: string): Promise<EstData>;
    downloadExcel(startDate?: string, endDate?: string, areaId?: string, estado?: string, subdireccionId?: string): Promise<{ blob: Blob; filename: string }>;
    downloadPdf(startDate?: string, endDate?: string, areaId?: string, estado?: string, subdireccionId?: string): Promise<{ blob: Blob; filename: string }>;
}
...
interface ReportesPageProps {
    ...
    kpiIcon: string;
    entityLabel: string;
    service: ReportesPageService;
}

// Genérico para las páginas "Reportes — <dominio>" (Articulaciones/IVC hoy, misma estructura
// de filtros/KPI/gráficas). ReportesSalidas.tsx queda aparte por tener filtros adicionales
// (jornada, tipo, subtipo).
export default function ReportesPage({ pageTitle, pageDescription, kpiIcon, entityLabel, service }: ReportesPageProps) {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await service.getEstadisticas(
    ...
    const handleClear = () => { setStartDate(''); setEndDate(''); setSelectedArea(''); setSelectedSubdireccion(''); setSelectedEstado(''); };
                        ...
                        <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div>
                                    <h1 className="text-3xl font-black text-zinc-900 tracking-tight flex items-center gap-3">
                                        <BarChart2 className="text-primary" size={32} />{pageTitle}
                                    </h1>
                                    <p className="text-zinc-500 mt-1">{pageDescription}</p>
                                </div>
                                <div className="flex items-center gap-3 flex-wrap">
                                    <button
                                        onClick={async () => { setIsGeneratingExcel(true); try { const r = await service.downloadExcel(startDate || undefined, endDate || undefined, selectedArea || undefined, selectedEstado || undefined, selectedSubdireccion || undefined); triggerDownload(r.blob, r.filename); } finally { setIsGeneratingExcel(false); } }}
                                    ...
                                    <button
                                        onClick={async () => { setIsGeneratingPdf(true); try { const r = await service.downloadPdf(startDate || undefined, endDate || undefined, selectedArea || undefined, selectedEstado || undefined, selectedSubdireccion || undefined); triggerDownload(r.blob, r.filename); } finally { setIsGeneratingPdf(false); } }}
                            ...
                            <div className="relative">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
                                    <div className="bg-primary p-6 rounded-2xl shadow-md flex items-center gap-5 relative overflow-hidden group">
                                        ...
                                        <div className="size-14 rounded-2xl bg-white/20 text-white flex items-center justify-center backdrop-blur-sm border border-white/10">
                                            <span className="material-symbols-outlined text-[32px]">{kpiIcon}</span>
                                        </div>
                                        <div className="z-10">
                                            <p className="text-indigo-100 font-medium text-sm mb-1">Total {entityLabel}</p>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                                            ...
                                            {!data?.topSolicitantes?.length ? (
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <BarChart data={data.topSolicitantes} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                        ...
                                                        <Bar dataKey="count" fill="#10b981" radius={[6, 6, 0, 0]} name={entityLabel} maxBarSize={50} isAnimationActive={false}>
                                    ...
                                    <div className="bg-white p-5 md:p-7 rounded-2xl border border-zinc-200/80 shadow-sm">
                                        <div className="flex items-center gap-2 mb-6">
                                            <span className="material-symbols-outlined text-amber-500 bg-amber-50 p-1.5 rounded-lg text-sm">donut_large</span>
                                        ...
                                        <div className="h-[300px]">
                                            {!data?.estados?.length ? (
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <BarChart data={data.estados} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f4f4f5" />
                                                        <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#71717A', fontWeight: 600 }} tickLine={false} axisLine={false} />
                                                        <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: '#A1A1AA' }} tickLine={false} axisLine={false} />
                                                        <RechartsTooltip cursor={{ fill: '#f4f4f5', radius: 6 }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgb(0 0 0/0.1)', padding: '12px 16px' }} />
                                                        <Bar dataKey="count" fill="#f59e0b" radius={[6, 6, 0, 0]} name={entityLabel} maxBarSize={50} isAnimationActive={false}>
                                                            {data.estados.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                                                        </Bar>
                                                    </BarChart>
                                                </ResponsiveContainer>
                                            )}
                                    ...
                                    <div className="bg-white p-5 md:p-7 rounded-2xl border border-zinc-200/80 shadow-sm lg:col-span-2">
                                        <div className="flex items-center gap-2 mb-6">
                                            ...
                                            <h3 className="text-lg font-bold text-zinc-800">{entityLabel} por Área</h3>
                                        </div>
                                            ...
                                            {!data?.areas?.length ? (
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <BarChart data={data.areas} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                        ...
                                                        <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: '#A1A1AA' }} tickLine={false} axisLine={false} />
                                                        <RechartsTooltip cursor={{ fill: '#f4f4f5', radius: 6 }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgb(0 0 0/0.1)', padding: '12px 16px' }} />
                                                        <Bar dataKey="count" fill="#6366f1" radius={[6, 6, 0, 0]} name={entityLabel} maxBarSize={60} isAnimationActive={false}>
                                    ...
                                    <div className="bg-white p-5 md:p-7 rounded-2xl border border-zinc-200/80 shadow-sm lg:col-span-2">
                                        <div className="flex items-center gap-2 mb-6">
                                            ...
                                            <h3 className="text-lg font-bold text-zinc-800">{entityLabel} por Subdirección</h3>
                                        </div>
                                        <div className="h-[350px]">
                                            {!data?.porSubdireccion?.length ? (
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <BarChart data={data.porSubdireccion} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                        ...
                                                        <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: '#A1A1AA' }} tickLine={false} axisLine={false} />
                                                        <RechartsTooltip cursor={{ fill: '#f4f4f5', radius: 6 }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgb(0 0 0/0.1)', padding: '12px 16px' }} />
                                                        <Bar dataKey="count" fill="#06b6d4" radius={[6, 6, 0, 0]} name={entityLabel} maxBarSize={60} isAnimationActive={false}>
