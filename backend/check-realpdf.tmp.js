...
(async () => {
    ...
    const asesoria = await prisma.asesorias.create({
        data: {
            codigo: 'TEST-REALPDF-' + Date.now(),
            fecha: new Date(), hora: '09:00', hora_fin: '10:00',
            medio: 'Presencial', institucion: 'Institucion de Prueba',
            temas_tratados: 'Prueba PDF real QR esquina', material_entregado: 'Ninguno',
    ...
    fs.writeFileSync('/tmp/certificado-real.pdf', buffer);
    console.log('PDF real generado:', buffer.length, 'bytes');
