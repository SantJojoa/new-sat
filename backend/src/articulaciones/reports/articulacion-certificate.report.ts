...
@Injectable()
export class ArticulacionCertificateReport {
    constructor(private readonly puppeteerBrowser: PuppeteerBrowserService) { }

    async generate(articulacion: any): Promise<Buffer> {
        const html = this.buildHtml(articulacion);
    ...
    private buildHtml(s: any): string {
        const seg = s.seguimiento_articulacion;
        const acta = seg ?? {};
